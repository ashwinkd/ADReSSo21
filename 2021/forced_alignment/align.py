...
def align_using_aeneas(audio_file, transcript):
    ...
    text_file = os.path.join("tmp", "aeneas_files", "text", "{}.txt".format(file_id))
    out_file = os.path.join("tmp", "aeneas_files", "out", "{}.json".format(file_id))
...
def align_using_penn(audio_file, transcript):
    ...
    text_file = os.path.join("tmp", "penn_files", "text", "{}.txt".format(file_id))
    wout_file = os.path.join("tmp", "penn_files", "word_out", "{}.pickle".format(file_id))
    pout_file = os.path.join("tmp", "penn_files", "phoneme_out", "{}.pickle".format(file_id))
    with open(text_file, 'w') as fptr:
