...
aligned_files = os.listdir(os.path.join('.', 'tmp', "aeneas_files", 'out'))
audio_files = os.listdir(os.path.join('.', 'tmp', "aeneas_files", 'audio'))
for file in r.sample(audio_files, len(audio_files)):
    audio = os.path.join('.', 'tmp', "aeneas_files", 'audio', file)
    ...
    with open(os.path.join('.', 'tmp', "aeneas_files", 'out', aligned_filename)) as f:
        data = json.load(f)
